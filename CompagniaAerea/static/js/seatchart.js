(function umd(root, factory) {
     ...
     *
     * @param {Array.<number>}[options.map.classe.seats] - Array of the other classe seats.
     * @param {Array.<number>}[options.map.classe.rows] - Array of the other classe rows of seats.
     * 
    ...
    function Seatchart(options) {
        ...
        } else if (options.map.rows > 99 || options.map.columns > 25) {
            throw new Error("Invalid parameter 'options.map' supplied to Seatchart. " +
                "'row' or 'columns' properties too big.");
        } else if (options.map.rows < 10 || options.map.columns < 2) {
            throw new Error("Invalid parameter 'options.map' supplied to Seatchart. " +
                "'row' or 'columns' properties too small.");
        }
        ...
        */
        ...
        var updateTotal = function updateTotal() {
            if (cartTotal !== undefined) {
                cartTotal.textContent = 'Totale: {0}{1}'.format(options.cart.currency, self.getTotal().toFixed(2));
            }
        ...
        var createTicket = function createTicker(seat) {
            ...
            seatName.id = 'seatName';
            seatName.textContent = seat.name;
            seatName.className = 'sc-cart-seat-name carrello';
        ...
        /**
         * Quando il cliente clicca sull'input "Concludi", qui si crea l'array che salva
         * i posti che sono stati selezionati e li passa a flask per la pagina "conferma"
         * per inserirli nel database.
         */

        var posti = [];
        $('#conferma').unbind('click').bind('click', function () { // quando viene cliccato "Concludi"
            $(".carrello").each(function () { // si prendono i biglietti creati nel carrello
                posti.push($(this).text()); // si salvano i posti selezionati
            })
            $("#biglietti").attr("value", posti.toString()); // si inviano i posti selezionati
        });
        ...
        */
        ...
        var createLegend = function createLegend() {
            ...
            seatsList.appendChild(createLegendItem('Disponibile', 'available'));
            seatsList.appendChild(createLegendItem('Prenotato', 'unavailable'));
            seatsList.appendChild(createLegendItem('Classe diversa', 'classe'));
        ...
        var createCart = function createCart() {
            ...
            var cartTitle = createTitle('Carrello');
